...
def send_email(Reserved_user_email, email_subject, email_body):
    ...
    smtp_password ="mgtn juwa pbco gphv"
    try:
...
def fetch_reservation_data():
    reservation_api_url='http://10.20.100.30:5001/reservation/viewall'
...
if __name__ == '__main__':
    ...
    logger.debug("Loading Swagger UI...")
    inventory_api_url = 'http://127.0.0.1:5001/inventory/view-all'
    #inventory_api_url='http://10.20.100.30:5001/inventory/view-all'
    response = requests.get(inventory_api_url)
    ...
    if response.status_code == 200:
        ...
        print("API request failed with status code:", response.status_code)
    #app.run(debug=True,host="10.20.100.30",port=5000)
